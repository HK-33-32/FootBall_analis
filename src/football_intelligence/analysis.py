...
class Analysis:
    ...
    def _run(self) -> None:
        ...
        for index, chunk in enumerate(chunks, start=1):
            ...
            # the rate this machine is actually managing, not the default
            self.progress.rate = (time.time() - perception_started) / max(analysed_s, 0.001)
...
class AnalysisManager:
    ...
    def submit(
        self, video: Path, title: str, roster: dict[str, Any] | None = None
    ) -> Analysis:
        analysis = Analysis(video, title, self.config, roster)
        with self._lock:
