    ...
    for image_id, predictions in image_id_to_predictions.items():
        ...
        for prediction in predictions:
            ...
            if role in ["referee", "other", "ball"]:
                ...
                # Goalkeepers wear a kit unlike either team, so they get their
                # own colour; the team they belong to stays in the label.
                team = str(attributes.get("team", "") or "")
                team_label = {"left": "L", "right": "R"}.get(team.lower(), "?")
                label_text = f"GK {team_label}"
                draw_color = (255, 0, 255)  # Magenta (BGR)
            else:
                ...
                if name:
                    label_text = f"{name} {jersey}" if str(jersey) != "100" else str(name)
                else:
            ...
            if bbox_pitch and bbox_pitch.get('x_bottom_middle') is not None:
