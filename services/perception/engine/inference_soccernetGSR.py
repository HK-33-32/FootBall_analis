...
class GSRPipeline:
    ...
    def _init_models(self):
        # 1) Detection model: RF-DETR or the original SoccerNet YOLOX-x
        det_cfg = dict(self.track_cfg)
        det_cfg.update(self.cfg.get('DETECTOR') or {})
        det_cfg.setdefault('TYPE', 'yolox')
        self.detector = build_detector(det_cfg, self.device,
                                       batch_size=self.cfg.get('BATCH_SIZE', 4))
        print(f"Detector: {self.detector.name}", flush=True)
        ...
        if self.jersey_mode == 'CLIP':
            ...
            self.jersey_batcher = BatchedJerseyClassifier(self.jersey_model, self.device)
        else:
    ...
    def process_video(self, video_path, vis_folder, batch_size=4):
        ...
        # windows spawns worker processes and cannot pickle a lambda collate_fn
        ...
        jersey_stride = max(1, int(self.cfg.get('JERSEY_STRIDE', 1)))
        jersey_cache = {}          # track_id -> (frame_index, (role, number, colour))
        frame_index = 0
            ...
            for i in range(len(frames)):
                ...
                for j, t in enumerate(online_targets):
                    ...
                    if x2_crop > x1_crop and y2_crop > y1_crop:
                        # Role / number / colour are aggregated per tracklet later on,
                        # so a track only has to be classified every JERSEY_STRIDE
                        # frames; in between we reuse its last prediction.
                        seen = jersey_cache.get(tid)
                        if (seen is not None and jersey_stride > 1
                                and frame_index - seen[0] < jersey_stride):
                            detection_predictions[j] = seen[1]
                        else:
